...
import styles from './SalesPageMobile.module.css';
...
const SalesPageMobile = () => {
    ...
    const [expandedCartAttributes, setExpandedCartAttributes] = useState(new Set()); // Set с ID товаров в корзине, у которых раскрыты атрибуты
    const [activeTab, setActiveTab] = useState('products'); // 'products' или 'carts'
                        ...
                        <div key={item.variantId} className={styles.cartItem}>
                            ...
                            <div className={styles.cartItemContent}>
                                ...
                                <div className={styles.cartItemTop}>
                                    <h5 className={styles.cartItemTitle}>{item.productName}</h5>
                                    <button
                                ...
                                {item.attributes && item.attributes.length > 0 && (
                                    <div className={styles.cartItemAttributes}>
                                        {(expandedCartAttributes.has(item.variantId) 
                                            ? item.attributes 
                                            : item.attributes.slice(0, 3)
                                        ).map(attr => (
                                            <span key={attr.id} className={styles.cartItemAttribute}>
                                                <span className={styles.attributeName}>{attr.category_attribute_name}:</span>
                                                <span className={styles.attributeValue}>{attr.custom_value || attr.predefined_value_name}</span>
                                            </span>
                                        ))}
                                        {item.attributes.length > 3 && (
                                            <button
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setExpandedCartAttributes(prev => {
                                                        const newSet = new Set(prev);
                                                        if (newSet.has(item.variantId)) {
                                            ...
                                            >
                                                {expandedCartAttributes.has(item.variantId) 
                                                    ? <>Скрыть <FaChevronUp /></>
                                                    : <>+{item.attributes.length - 3} еще <FaChevronDown /></>
                                                }
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div className={styles.cartItemMeta}>
                                    <span className={styles.cartItemSku}>Артикул: {item.sku}</span>
                                    {item.unitDisplay && (
                                        <span className={styles.cartItemUnit}>• {item.unitDisplay}</span>
                                    )}
                                </div>
                                ...
                                {/* Нижняя часть: количество, цена, действия */}
                                <div className={styles.cartItemBottom}>
                                    <div className={styles.cartItemBottomLeft}>
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.tabs}>
                ...
                >
                    <FaBoxOpen /> Корзины
                    {(cart.length > 0 || cart2.length > 0) && (
                        <span className={styles.tabBadge}>{cart.length + cart2.length}</span>
                    )}
                </button>
            </div>
                        ...
                        <div className={styles.searchSection}>
                            <div className={styles.searchInputGroup}>
                                ...
                                <button
                                    ...
                                    disabled={!selectedLocation}
                                >
                                    {scannerActive ? <FaCheck /> : <FaBarcode />}
                                </button>
                                <button
                    ...
                    <div className={styles.cartsTab}>
                        ...
                        <div className={styles.cartCard}>
                            <div className={styles.cartHeader}>
                                <h4>Корзина 2</h4>
                                {cart2.length > 0 && (
                                    <span className={styles.cartCount}>{cart2.length}</span>
                                )}
                            </div>
