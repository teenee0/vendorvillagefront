...
import { FaPlusCircle, FaImages, FaInfoCircle, FaListUl, FaCloudUploadAlt, FaStar, FaTimes, FaTrash, FaPlus, FaCopy, FaSave, FaMagic, FaImage } from 'react-icons/fa';
...
/** Синхронно с ai.services.product_image_background.MAX_BACKGROUND_PROMPT_LENGTH */
export const AI_BACKGROUND_PROMPT_MAX = 100;
/** Синхронно с MAX_IMAGES_PER_REQUEST в product_image_background */
export const AI_BACKGROUND_MAX_IMAGES = 20;
...
const AttributeValueSelect = ({
  ...
  return (
    <div
      className={`${styles.attrSelectWrap}${disabled ? ` ${styles.attrSelectWrapDisabled}` : ''}`}
      ref={rootRef}
    >
      <div
...
/** Для ИИ: у существующих фото с сервера может не быть `file` — подтягиваем по URL превью */
export async function resolveImageFileForAi(img) {
...
const ProductAddPage = () => {
  ...
  /** replace_ai: обрезка 5:7 после ответа ИИ по существующим id слотов */
  ...
  /** Предпросмотр фото от ИИ (фон): до принятия галерея не меняется */
  const [aiBgReview, setAiBgReview] = useState(null);
  const aiBgReviewRef = useRef(null);

  // Варианты товара (attributes — только at_right); общие атрибуты — sharedBaseAttributes
  const [variants, setVariants] = useState([]);
  const [sharedBaseAttributes, setSharedBaseAttributes] = useState({});
  /** Для предустановленных at_right: какие id значений справочника участвуют в вариантах */
  ...
  /** Только в dev: заблокировать те же поля, что при ИИ, без запроса */
  const [isAiFillOverlayDemo, setIsAiFillOverlayDemo] = useState(false);
  /** Блокируем только поля, которые ИИ перезаписывает (не всю форму). */
  ...
  // Загрузка товара (режим редактирования)
  useEffect(() => {
  ...
  const refreshAiQuota = useCallback(async () => {
    try {
      ...
      /* сохраняем предыдущее значение квоты при фоновом обновлении */
    }
  }, [business_slug]);
  ...
  // Добавление строки варианта (только режим со свободным текстом / смешанным at_right)
  ...
  // Копирование последнего варианта (только режим free at_right)
      ...
      <div className={styles.container}>
        {aiBgReview && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="ai-bg-review-title"
          >
            <div className={styles.aiBgReviewPanel}>
              <h3 id="ai-bg-review-title" className={styles.aiBgReviewTitle}>
              ...
              <PreviewableImage.PreviewGroup preview={{ zIndex: 3200, mask: 'Просмотр' }}>
        ...
        <div className={styles.content}>
          <h2 className={styles.pageTitle}>
            <FaPlusCircle className={styles.titleIcon} />{' '}
          ...
          <form onSubmit={handleSubmit} className={styles.productForm}>
              ...
              <div className={styles.sectionBody}>
                ...
                <div className={styles.aiFillRow}>
                    ...
                    <AiBackgroundPromptField
                      textareaId="ai-background-prompt"
                      businessSlug={business_slug}
                      ...
                      hint={
                        <p className={styles.aiFillHint} style={{ marginTop: '0.35rem' }}>
                          Обязательное поле: для каждого фото вызывается xAI Imagine (фон по описанию; товар по возможности
                          сохраняется). До {AI_BACKGROUND_PROMPT_MAX} символов. За один запрос не более{' '}
                          {AI_BACKGROUND_MAX_IMAGES} файлов; несколько фото — по очереди (может занять несколько минут).
                        </p>
                      }
                    />
                ...
                <div className={styles.aiFillRow}>
                    ...
                    <p className={styles.aiFillHint}>
                      Подставятся название, описание, общие атрибуты товара и значения атрибутов комплектации «справа» для первой строки варианта; при полной сетке комбинаций правятся только общие поля (черновик, проверьте перед сохранением). Сначала выберите категорию в блоке ниже.
                    </p>
                    {aiFillFieldsLocked && (
                      <p className={styles.aiFillFieldsLockedHint} role="status">
                        {isAiFillLoading
                          ? 'Временно недоступны: название, описание, общие атрибуты в блоке вариантов и поля комплектации у первой строки варианта (в режиме ручных вариантов). Остальное можно менять.'
                          : 'Тест: заблокированы те же поля, что и при реальном запросе к ИИ.'}
                      </p>
                    )}
                  </div>
                {import.meta.env.DEV && (
                  <div className={styles.aiFillDemoRow}>
                    ...
                    >
                      {isAiFillOverlayDemo
                        ? 'Снять тест блокировки полей ИИ'
                        : 'Тест: заблокировать поля как при ИИ'}
                    </button>
                    <span className={styles.aiFillDemoNote}>только в режиме разработки</span>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className={styles.sectionBody}>
                  <div className={styles.infoAlert}>
                    {variantMatrixMode === 'none' && (
                      <>У этой категории нет атрибутов «справа» на карточке — создаётся один вариант товара (штрих-код и далее остатки — при работе с товаром).</>
                    )}
                    {variantMatrixMode === 'cartesian' && (
                      <>Сверху отметьте значения комплектации — строки строятся из отмеченных комбинаций. Нельзя больше {ENV_CONFIG.PRODUCT_ADD_MAX_VARIANTS_MATRIX} вариантов. Общие атрибуты одинаковы для всех строк; штрих-код можно задать по строкам.</>
                    )}
                    {variantMatrixMode === 'free' && (
                      <>До {ENV_CONFIG.PRODUCT_ADD_MAX_VARIANTS_FREE_RIGHT_ATTR} вариантов: задайте комбинации атрибутов комплектации вручную. Общие атрибуты заполняются один раз и действуют для всех вариантов.</>
                    )}
                  </div>
                      ...
                      {variantMatrixMode === 'cartesian' && rightPredefinedAttrs.length > 0 && (
                        <div className={styles.attributeSection}>
                          ...
                          <p className={styles.attributeSectionHint}>
                            Отметьте значения по каждому атрибуту — например только M и L, без S и XL. Комбинации считаются из отмеченных значений.
                          </p>
                      ...
                      {baseAttrs.length > 0 && (
                        <div className={styles.variantCard} style={{ marginBottom: '1rem' }}>
                          <div className={styles.variantCardHeader}>
                            <span className={styles.variantCardTitle}>Общие атрибуты товара</span>
                          </div>
                      ...
                      <div className={styles.variantStack}>
                        {variants.length === 0 ? (
                          <div className={styles.noVariants}>
                            Нет вариантов для этой категории (слишком много комбинаций или нет значений в справочниках). Выберите другую категорию.
                          </div>
                        ) : (
              ...
              <div className={styles.sectionCard}>
                ...
                <div className={styles.sectionBody}>
                  <p className={styles.addProductVariantsPlaceholder}>
                    Выберите категорию в блоке «Основная информация», чтобы настроить варианты и атрибуты.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className={styles.formActions}>
              ...
              <button
                ...
                disabled={isSubmitting || isAiFillLoading || isAiBackgroundLoading || !!aiBgReview}
                style={{
                  opacity: isSubmitting || isAiFillLoading || isAiBackgroundLoading || !!aiBgReview ? 0.6 : 1,
                  cursor: isSubmitting || isAiFillLoading || isAiBackgroundLoading || !!aiBgReview ? 'not-allowed' : 'pointer',
                }}
              >
                {isSubmitting ? (
                  <>
